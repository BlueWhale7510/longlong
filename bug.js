class Bug {
    constructor(canvasWidth, canvasHeight) {
        this.types = [
            '高数', '程序设计基础', '大学英语', '大学物理', '程序设计进阶', '概率论与数理统计',
            '线性代数', '离散数学', '数字电子技术', '面向对象的程序设计', '计算机组成原理', '数据结构',
            '人工智能', '数据库原理', '操作系统与Linux', '机器学习', '算法设计与分析', '现代控制工程'
        ];
    ...
    draw(ctx) {
        ...
        switch (this.type) {
            case '高数': this.drawDeadlock(ctx); break;
            case '程序设计基础': this.drawMemoryLeak(ctx); break;
            case '大学英语': this.drawOutOfBounds(ctx); break;
            case '大学物理': this.drawNullPointer(ctx); break;
            case '程序设计进阶': this.drawException(ctx); break;
            case '概率论与数理统计': this.drawInjection(ctx); break;
            case '线性代数': this.drawInfiniteLoop(ctx); break;
            case '离散数学': this.drawTypeError(ctx); break;
            case '数字电子技术': this.drawConcurrencyConflict(ctx); break;
            case '面向对象的程序设计': this.drawResourceLeak(ctx); break;
            case '计算机组成原理': this.drawStackOverflow(ctx); break;
            case '数据结构': this.drawStackOverflow(ctx); break;
            case '人工智能': this.drawStackOverflow(ctx); break;
            case '数据库原理': this.drawStackOverflow(ctx); break;
            case '操作系统与Linux': this.drawStackOverflow(ctx); break;
            case '机器学习': this.drawStackOverflow(ctx); break;
            case '算法设计与分析': this.drawStackOverflow(ctx); break;
            case '现代控制工程': this.drawStackOverflow(ctx); break;
            // 可以为其他 BUG 类型添加特殊行为
        }
    ...
    drawTypeError(ctx) {
        ...
        ctx.textAlign = 'center';
        ctx.textBaseline = 'middle';
        ctx.fillText('Fail', centerX, centerY - this.height / 6);
        ctx.fillText('Exam', centerX, centerY + this.height / 6);
